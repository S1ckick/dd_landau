...
(define (cr+ . x) (match (target-real-implementation TARGET)
('double-double (format "dd_add(~a)" (string-join (map (lambda (y) (format "~a" y)) x) " , ")))
('long-double (format "(~a)" (string-join (map (lambda (y) (format "~a" y)) x) " + ")))
('double (format "(~a)" (string-join (map (lambda (y) (format "~a" y)) x) " + ")))
))

(define (crdl+ . x) (match (target-real-implementation TARGET)
('double-double (format "dd_add_d_dd(~a)" (string-join (map (lambda (y) (format "~a" y)) x) " , ")))
('long-double (format "(~a)" (string-join (map (lambda (y) (format "~a" y)) x) " + ")))
('double (format "(~a)" (string-join (map (lambda (y) (format "~a" y)) x) " + ")))
))

(define (crdr+ . x) (match (target-real-implementation TARGET)
('double-double (format "dd_add_dd_d(~a)" (string-join (map (lambda (y) (format "~a" y)) x) " , ")))
('long-double (format "(~a)" (string-join (map (lambda (y) (format "~a" y)) x) " + ")))
('double (format "(~a)" (string-join (map (lambda (y) (format "~a" y)) x) " + ")))
))
...
(define (crdl- x y) (match (target-real-implementation TARGET)
('double-double (format "dd_sub_d_dd(~a , ~a)" x y))
('long-double (format "(~a - ~a)" x y))
('double (format "(~a - ~a)" x y))
))

(define (crdr- x y) (match (target-real-implementation TARGET)
('double-double (format "dd_sub_dd_d(~a , ~a)" x y))
('long-double (format "(~a - ~a)" x y))
('double (format "(~a - ~a)" x y))
))
...
(define (cr* . x) (match (target-real-implementation TARGET)
('double-double (format "dd_mul(~a)" (string-join (map (lambda (y) (format "~a" y)) x) " , ")))
('long-double (format "(~a)" (string-join (map (lambda (y) (format "~a" y)) x) " * ")))
('double (format "(~a)" (string-join (map (lambda (y) (format "~a" y)) x) " * ")))
))

(define (crdl* . x) (match (target-real-implementation TARGET)
('double-double (format "dd_mul_d_dd(~a)" (string-join (map (lambda (y) (format "~a" y)) x) " , ")))
('long-double (format "(~a)" (string-join (map (lambda (y) (format "~a" y)) x) " * ")))
('double (format "(~a)" (string-join (map (lambda (y) (format "~a" y)) x) " * ")))
))

(define (crdr* . x) (match (target-real-implementation TARGET)
('double-double (format "dd_mul_dd_d(~a)" (string-join (map (lambda (y) (format "~a" y)) x) " , ")))
('long-double (format "(~a)" (string-join (map (lambda (y) (format "~a" y)) x) " * ")))
('double (format "(~a)" (string-join (map (lambda (y) (format "~a" y)) x) " * ")))
))
...
(define (cr/ x y) (match (target-real-implementation TARGET)
('double-double (format "dd_div(~a , ~a)" x y))
('long-double (format "(~a / ~a)" x y))
('double (format "(~a / ~a)" x y))
))

(define (crdl/ x y) (match (target-real-implementation TARGET)
('double-double (format "dd_div_d_dd(~a , ~a)" x y))
('long-double (format "(~a / ~a)" x y))
('double (format "(~a / ~a)" x y))
))

(define (crdr/ x y) (match (target-real-implementation TARGET)
('double-double (format "dd_div_dd_d(~a , ~a)" x y))
('long-double (format "(~a / ~a)" x y))
('double (format "(~a / ~a)" x y))
))
...
(define (c-neg x) (match (target-real-implementation TARGET)
('double-double (format "dd_neg(~a)" x))
('long-double (format "-~a" x))
('double (format "-~a" x))
))
