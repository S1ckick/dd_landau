...
void bodies_to_c(dd * state, std::vector<Body<Type>> &bodies){
  for(int i = 0; i < bodies.size(); i++){
    dd mass = {bodies[i].m.x[0], bodies[i].m.x[1]};
    state[i*7] = mass;

    dd r1 = {bodies[i].r.X.x[0], bodies[i].r.X.x[1]};
    dd r2 = {bodies[i].r.Y.x[0], bodies[i].r.Y.x[1]};
    dd r3 = {bodies[i].r.Z.x[0], bodies[i].r.Z.x[1]};
    state[i*7 + 1] = r1;
    state[i*7 + 2] = r2;
    state[i*7 + 3] = r3;

    dd v1 = {bodies[i].v.X.x[0], bodies[i].v.X.x[1]};
    dd v2 = {bodies[i].v.Y.x[0], bodies[i].v.Y.x[1]};
    dd v3 = {bodies[i].v.Z.x[0], bodies[i].v.Z.x[1]};
    state[i*7 + 4] = v1;
    state[i*7 + 5] = v2;
    state[i*7 + 6] = v3;
  }
...
template <typename Type>
void c_to_bodies(dd * state, std::vector<Body<Type>> &bodies){
  for(int i = 0; i < bodies.size(); i++){
    bodies[i].m = Type(state[i*7].x, state[i*7].y);
    bodies[i].r = vec<Type>(Type(state[i*7 + 1].x, state[i*7 + 1].y), Type(state[i*7 + 2].x, state[i*7 + 2].y), Type(state[i*7 + 3].x, state[i*7 + 3].y) );
    bodies[i].v = vec<Type>(Type(state[i*7 + 4].x, state[i*7 + 4].y), Type(state[i*7 + 5].x, state[i*7 + 5].y), Type(state[i*7 + 6].x, state[i*7 + 6].y) );
  }
}
...
void RungeKutta4(std::vector<Body<Type>> &bodies, Type h) {
    ...
    bodies_to_c(dd_temp, bodies);
    newState(dd_temp_res, dd_temp);
    c_to_bodies(dd_temp_res, k_1);
    ...
    bodies_to_c(dd_temp, temp);
    newState(dd_temp_res, dd_temp);
    c_to_bodies(dd_temp_res, k_2);
...
std::vector<Type> initDDCoef() {
  Type  b1,b6,b7,b8,b9,b10,b11,b12,
      a21,a31,a32,a41,a43,a51,a53,
      a54,a61,a64,a65,a71,a74,a75,a76,
      a81,a84,a85,a86,a87,a91,a94,a95,
      a96,a97,a98,a101;
  Type a104,a105,a106,a107,a108,a109,
      a111,a114,a115,a116,a117,a118,
      a119,a1110,a121,a124,a125,a126,
    ...
    Type a104("-2.48811461997166764192642586468E0"),
...
template <typename Type>
void dormanPrince8(std::vector<Body<Type>> &bodies, Type h, std::vector<Type> &coefs) {
    ...
    copyBodies(bodies + (k_1*coefs[5] + k_3*coefs[6] + k_4*coefs[7])*h,temp);
    ...
    bodies_to_c(dd_temp, temp);
    newState(dd_temp_res, dd_temp);
    c_to_bodies(dd_temp_res, k_5);

    //temp = bodies + h*( a61*k1 + a64*k4 + a65*k5)
    copyBodies(bodies + (k_1*coefs[8] + k_4*coefs[9] + k_5*coefs[10])*h,temp);
    ...
    bodies_to_c(dd_temp, temp);
    newState(dd_temp_res, dd_temp);
    c_to_bodies(dd_temp_res, k_6);

    //temp = bodies + h*( a71*k1 + a74*k4 + a75*k5 + a76*k6)
    copyBodies(bodies + (k_1*coefs[11] + k_4*coefs[12] + k_5*coefs[13] + k_6*coefs[14])*h,temp);
    ...
    bodies_to_c(dd_temp, temp);
    newState(dd_temp_res, dd_temp);
    c_to_bodies(dd_temp_res, k_7);

    //temp = bodies + h*( a81*k1 + a84*k4 + a85*k5 + a86*k6 + a87*k7)
    copyBodies(bodies + (k_1*coefs[15] + k_4*coefs[16] + k_5*coefs[17] + k_6*coefs[18] + k_7*coefs[19])*h,temp);
    ...
    bodies_to_c(dd_temp, temp);
    newState(dd_temp_res, dd_temp);
    c_to_bodies(dd_temp_res, k_8);

    //temp = bodies + h*( a91*k1 + a94*k4 + a95*k5 + a96*k6 + a97*k7 + a98*k8)
    copyBodies(bodies + (k_1*coefs[20] + k_4*coefs[21] + k_5*coefs[22] + k_6*coefs[23] + k_7*coefs[24] + k_8*coefs[25])*h,temp);
    ...
    bodies_to_c(dd_temp, temp);
    newState(dd_temp_res, dd_temp);
    c_to_bodies(dd_temp_res, k_9);

    //temp = bodies + h*( a101*k1 + a104*k4 + a105*k5 + a106*k6 + a107*k7 + a108*k8 + a109*k9)
    copyBodies(bodies + (k_1*coefs[26] + k_4*coefs[27] + k_5*coefs[28] + k_6*coefs[29] + k_7*coefs[30] + k_8*coefs[31] + k_9*coefs[32])*h,temp);
    ...
    bodies_to_c(dd_temp, temp);
    newState(dd_temp_res, dd_temp);
    c_to_bodies(dd_temp_res, k_10);

    //temp = bodies + h*( a111*k1 + a114*k4 + a115*k5 + a116*k6 + a117*k7 + a118*k8 + a119*k9 + a1110*k_10)
    copyBodies(bodies + (k_1*coefs[33] + k_4*coefs[34] + k_5*coefs[35] + k_6*coefs[36] + k_7*coefs[37] + k_8*coefs[38] + k_9*coefs[39] + k_10*coefs[40])*h,temp);
    ...
    copyBodies(bodies + (k_1*coefs[41] + k_4*coefs[42] + k_5*coefs[43] + k_6*coefs[44] + k_7*coefs[45] + k_8*coefs[46] + k_9*coefs[47] + k_10*coefs[48] + k_2*coefs[49])*h,temp);
    ...
    copyBodies(k_1*coefs[50] + k_6*coefs[51] + k_7*coefs[52] + k_8*coefs[53] + k_9*coefs[54] + k_10*coefs[55] + k_2*coefs[56] + k_3*coefs[57], k_4);
